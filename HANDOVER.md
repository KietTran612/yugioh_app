...
└── yugioh_card_app/
    ...
    │   │   ├── favorites_service.dart   # Persist/load favorites via SharedPreferences
    ...
    │       ├── favorite_button.dart     # Bookmark toggle button
    │       ├── filter_panel.dart        # Bottom sheet filter
    │       └── quick_filter_bar.dart
    │   └── utils/
    │       ├── app_theme.dart           # AppTheme — colors, text styles, constants
    │       └── card_colors.dart         # Frame type / attribute color mapping
    └── pubspec.yaml
```
...
- **Card Picker Screen** (`card_picker_screen.dart`) — màn hình browse/add card vào deck
  ...
  - Tap card → `QuickAddSheet` (`widgets/card_picker_sheet.dart`)
